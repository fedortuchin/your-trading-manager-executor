...
submits the stop-loss as a market close after trigger. OKX documents this precheck endpoint as
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.5/scripts/install.sh | sudo bash -s -- \
...
pinned public executor image `ghcr.io/fedortuchin/your-trading-manager-executor:v0.7.5`, enrolls it
...
```bash
curl -fsSLO https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.5/docker-compose.yml
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.5/scripts/install-venv.sh | sudo bash -s -- \
...
pip install \
  ...
  ytm-executor-0.7.5-py3-none-any.whl
```
...
with a sanitized `providerOrderId`.
...
```bash
scripts/sign_artifact.sh dist/ytm_executor-0.7.5-py3-none-any.whl
```
...
Production installs should use a version tag such as `v0.7.5` or an image digest such as
