...
pinned public executor image `ghcr.io/fedortuchin/your-trading-manager-executor:v0.6.0`, enrolls it
...
```bash
curl -fsSLO https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.6.0/docker-compose.yml
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.6.0/scripts/install-venv.sh | sudo bash -s -- \
...
pip install \
  ...
  ytm-executor-0.6.0-py3-none-any.whl
```
...
For OKX SWAP, use `okx_swap` as the local market. `allowedSymbols` must match the symbol that
YTM sends in the command. The adapter accepts native OKX ids like `BTC-USDT-SWAP` and also maps
plain USDT pairs like `BTCUSDT` to OKX SWAP instrument ids before broker precheck. If a command
contains `quantity`, OKX treats it as contract size; otherwise the adapter can derive contract size
from `orderNotional` plus `priceReference`:
...
```bash
scripts/sign_artifact.sh dist/ytm_executor-0.6.0-py3-none-any.whl
```
...
Production installs should use a version tag such as `v0.6.0` or an image digest such as
