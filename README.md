...
pinned public executor image `ghcr.io/fedortuchin/your-trading-manager-executor:v0.7.8`, enrolls it
...
```bash
curl -fsSLO https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.8/docker-compose.yml
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.8/scripts/install-venv.sh | sudo bash -s -- \
...
pip install \
  ...
  ytm-executor-0.7.8-py3-none-any.whl
```
...
`providerOrderId` and `protectionStatus`.
...
```bash
scripts/sign_artifact.sh dist/ytm_executor-0.7.8-py3-none-any.whl
```
...
Production installs should use a version tag such as `v0.7.8` or an image digest such as
