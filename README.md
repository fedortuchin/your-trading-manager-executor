...
`order_placement_skipped`. For `real`, the separate `okx_swap_mainnet_order` adapter requires an
entry stop-loss, attaches it to the entry order through OKX `attachAlgoOrds`, first calls the same
`order-precheck`, then calls `POST /api/v5/trade/order` only when all local gates pass and the
executor was started with `--enable-real-orders`. Attached stop-loss uses `slOrdPx=-1`, so OKX
submits the stop-loss as a market close after trigger. After submit, the executor verifies the
active OKX algo stop-loss. If an open position exists without a matching stop-loss, it attempts a
standalone reduce-only conditional stop-loss. If that remediation fails, the command result is
reported with `protectionStatus=unprotected` for operator action. OKX documents this precheck
endpoint as Trade-permission and only applicable to multi-currency margin mode and portfolio margin
mode; simple-mode accounts may fail closed at broker precheck.
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.6/scripts/install.sh | sudo bash -s -- \
...
pinned public executor image `ghcr.io/fedortuchin/your-trading-manager-executor:v0.7.6`, enrolls it
...
```bash
curl -fsSLO https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.6/docker-compose.yml
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.6/scripts/install-venv.sh | sudo bash -s -- \
...
pip install \
  ...
  ytm-executor-0.7.6-py3-none-any.whl
```
...
real trading, it requires an opening-order `stopLoss`, attaches it to the entry order through OKX
`attachAlgoOrds`, runs `order-precheck`, then calls `trade/order` and reports `order_submitted`
with a sanitized `providerOrderId` and `protectionStatus`. If attached stop-loss verification
fails while a position is already open, the executor attempts a standalone reduce-only stop-loss
and reports whether protection was remediated or remains unprotected.
...
```bash
scripts/sign_artifact.sh dist/ytm_executor-0.7.6-py3-none-any.whl
```
...
Production installs should use a version tag such as `v0.7.6` or an image digest such as
