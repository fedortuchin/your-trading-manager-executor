...
mainnet and calls `test_order` only; Binance validates the request without submitting it to the
matching engine, and the executor still rejects real order placement after that validate-only call.
...
pinned public executor image `ghcr.io/fedortuchin/your-trading-manager-executor:v0.4.0`, enrolls it
...
```bash
curl -fsSLO https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.4.0/docker-compose.yml
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.4.0/scripts/install-venv.sh | sudo bash -s -- \
...
pip install \
  ...
  ytm-executor-0.4.0-py3-none-any.whl
```
...
which calls Binance USD-M Futures mainnet `test_order` only and reports `order_test_validated` when
Binance accepts the validate-only request. The executor still rejects the command with
`real_execution_disabled` after this preflight because real placement is not enabled yet.
...
```bash
scripts/sign_artifact.sh dist/ytm_executor-0.4.0-py3-none-any.whl
```
...
Production installs should use a version tag such as `v0.4.0` or an image digest such as
