...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.11/scripts/install.sh | sudo bash -s -- \
...
pinned public executor image `ghcr.io/fedortuchin/your-trading-manager-executor:v0.7.11`, enrolls it
...
```bash
curl -fsSLO https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.11/docker-compose.yml
...
```bash
curl -fsSL https://raw.githubusercontent.com/fedortuchin/your-trading-manager-executor/v0.7.11/scripts/install-venv.sh | sudo bash -s -- \
...
pip install \
  ...
  ytm-executor-0.7.11-py3-none-any.whl
```
...
```bash
scripts/sign_artifact.sh dist/ytm_executor-0.7.11-py3-none-any.whl
```
...
Production installs should use a version tag such as `v0.7.11` or an image digest such as
